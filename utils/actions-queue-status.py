...
def stored_repos_file(reporter: "Reporter", args: argparse.Namespace) -> Path | None:
    ...
    if args.delete_cached_projects:
        reporter.warn(f"Rediscovering {args.org} and rewriting {path.name}")
        return None
    if args.include_archived:
        ...
        reporter.warn(f"{path.name} holds no archived repos — discovering afresh")
        return None
    return path
...
def main() -> int:
    ...
    stored = None if args.repos_file else stored_repos_file(reporter, args)
    if args.repos_file or stored:
        source = args.repos_file or stored
        repos = read_repos_file(source)
        reporter.log(f"Loaded [bold]{len(repos)}[/] repos from {escape(display_path(source))}", "cyan")
        age = repos_file_age_days(Path(source))
        if age is not None and age > STALE_AFTER_DAYS:
            reporter.warn(
                f"That list was discovered {age} days ago — repos that have adopted "
                f"Actions since are missing from this sweep. Refresh it with "
                f"--delete-cached-projects."
            )
        elif age is not None:
            reporter.detail(f"Discovered {age} days ago — --delete-cached-projects refreshes it.")
    else:
        ...
        if args.delete_cached_projects:
            ...
            reporter.success(f"Wrote {display_path(stored_path)} ({len(repos)} repos)")
        if args.save_repos:
