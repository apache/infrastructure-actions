...
| #802 | carabiner-dev nested `install/{ampel,bnd}` do `curl + chmod 0755`; upstream ships SLSA `provenance.json` / `attestations.jsonl` | C | Upstream issue carabiner-dev/actions#51 + PR comment. **Resolved upstream — see the #1279-#1292 row; do not re-raise this against current versions** |
...
| #1279-#1292 | Five carabiner-dev bumps in one sweep (`install/{bnd,ampel,ampel-bootstrap,download-and-verify}`, `ampel/verify`). The case-C shape from #802 is **gone**: carabiner-dev/actions#51 is closed and `install/download-and-verify` now defaults `verify` to true, checking SLSA provenance via a bootstrapped ampel trust anchor against a pinned policy revision with a sigstore signer spec. Internal references also moved from a separately-pinned `…@d6d91c5c` to the `$/` self-repository form, so one approved SHA now covers the whole install chain instead of an independently floating pin | clean | Approved and merged. **Two of the five (`ampel-bootstrap`, `download-and-verify`) had a byte-identical `action.yml` at old and new pin — compare blob SHAs rather than trusting the compare's file list, which listed unrelated commits** |
| #1283 | `benchmark-action/github-action-benchmark` v1.22.2 — `Vendored npm registry check ✗ 1 modified`: `node-fetch`'s nested `whatwg-url@5.0.0` ships `lib/url-state-machine.js` with **CRLF**, git normalises to LF on commit, so the committed blob can never byte-match the tarball (33573 bytes / 1297 CRs published; 32276 / none committed; `33573 - 32276 = 1297`). Folding CRLF to LF hashes to exactly the committed blob | E | Fix in #1297 (retry the byte comparison with `\r\n` folded, credit as `crlf_normalized`). **Before concluding, check content for injection and compare the blob against the approved version — `diff --strip-trailing-cr` settles it in one command** |
| #1298 | `loadingalias/cargo-rail-action@e706767c` (v10.0.0), a *new* action from an external contributor. Provenance checks out — steady v3→v10 cadence, MIT, and the annotated tag dereferences to exactly the pinned SHA. `verify` passes everything. But the composite's payload, `scripts/bootstrap.sh`, downloads the runtime from the GitHub release and verifies it against a manifest fetched **from that same release**: integrity, not authenticity. `release.yml` has no `id-token: write` and no signing step | B | Held; upstream issue loadingalias/cargo-rail-action#3 proposing `actions/attest-build-provenance` + PR comment. **`verify` reports "no downloads or all verified" whenever checksum verification is *present* — it does not reason about where the checksum came from, so read the install script yourself. Same shape as the mise-action row (#1013)** |
| #1319 | `google/oss-fuzz/infra/cifuzz/actions/{build_fuzzers,run_fuzzers}@8f63412`, a *new* action pair from an external contributor (upstream publishes no tags, so the bare hash is correct). Everything green — but `Dockerfile analysis ⊘ no Dockerfile` on a **docker action that has one**. `analyze_dockerfile()` probed only `<sub_path>/Dockerfile` and the repo root, never reading `runs.image:` — here `'../../../build_fuzzers.Dockerfile'`, renamed *and* three levels above the action dir. Every base-image and command check silently skipped, hiding `FROM gcr.io/oss-fuzz-base/cifuzz-base` (untagged, so `:latest`). `verification.py` carried the same root-only assumption when choosing `skip` vs `pass`. The untagged base is **carried over**: the approved `google/clusterfuzzlite/actions/*` run `docker://gcr.io/oss-fuzz-base/clusterfuzzlite-build-fuzzers:v1` — same mutable gcr.io trust root, and #1319 is tighter (its infra layer *is* pinned by the commit; only the base floats) | E (+ carried-over D) | Approved. Fix in branch `verify-resolve-named-dockerfile`: resolve `runs.image:` into the candidate list, ignore `..` that climbs past the repo root, leave the `docker://` path intact. **A docker action's Dockerfile lives wherever `runs.image:` says — `⊘ no Dockerfile` on a docker action is a finding about the tool, not the action** |
